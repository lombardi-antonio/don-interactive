...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-screen-full h-14 vetro-glass ">
        <div className="flex flex-wrap items-center justify-between mx-auto py-3 px-6 lg:max-w-[900px] xl:max-w-[1500px]">
      ...
      >
        <ul className="flex flex-col p-2 font-medium vetro-glass rounded-b-xl">
          {links.map(({ href, label }) => (
            <li key={href} >
              <Link
                href={href}
                onClick={() => setIsVisible(false)}
                className="uppercase hover:cursor-pointer block py-2 pl-6 pr-4 text-gray-900 rounded-full hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white dark:border-gray-700"
              >
                {label}
              </Link>
            </li>
